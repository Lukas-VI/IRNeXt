"""
valid.py：训练过程中的验证流程（GoPro 数据集，在 valid 子集上计算平均 PSNR）。
与去雾版的 valid.py 同构，仅验证集变量名用 gopro、打印提示为 GoPro。
...
def _valid(model, args, ep):
    ...
    gopro = valid_dataloader(args.data_dir, batch_size=1, num_workers=0)  # 验证集(用 valid 子集)
    model.eval()     # 推理模式
    psnr_adder = Adder()
