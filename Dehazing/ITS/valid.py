"""
valid.py：训练过程中的验证流程（在验证集/test 子集上计算平均 PSNR）。
与 eval.py 类似但更轻量，只算 PSNR（不算 SSIM、不保存图），
...
def _valid(model, args, ep):
    ...
    its = valid_dataloader(args.data_dir, batch_size=1, num_workers=0)  # 验证集(用 test 子集)
    model.eval()     # 推理模式
    psnr_adder = Adder()
